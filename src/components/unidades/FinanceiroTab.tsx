// src/components/unidades/FinanceiroTab.tsx
...
type Resumo = { competencia: string; kpis: { totalDevido: number; totalRecebido: number; inadimplentes: number; inadimplenciaPct: number }; pendencias: Pendencia[] }

export default function FinanceiroUnidadeTab({ unidadeId }: { unidadeId: string }) {
  const [competencia, setCompetencia] = useState(defaultYm())
    ...
    <div className="space-y-4">
      <div className="flex gap-3 items-end">
        <div>
          ...
          <input type="month" value={ym} onChange={e => setCompetencia(e.target.value+'-01')}
...
const defaultYm = () => { const d=new Date(); const y=d.getUTCFullYear(); const m=String(d.getUTCMonth()+1).padStart(2,'0'); return `${y}-${m}-01` }
...
function Kpi({title, v}:{title:string; v:number}){ return <div className="rounded-2xl border p-4 shadow-sm bg-card"><div className="text-xs text-muted-foreground">{title}</div><div className="text-2xl mt-1 font-semibold">{brl(v)}</div></div> }
